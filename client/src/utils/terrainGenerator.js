 ...
 * ES module mirror of server/src/utils/terrainGenerator.js.
 * Must produce identical output given same inputs.
 */
...
export function getGridSize(planetSize) {
...
export function generateTerrain(colonyId, planetId, planetType, planetSize) {
...
export function generateDeposits(colonyId, planetId, planetType, planetSize, existingTerrain = null) {
  ...
  for (const [name, info] of Object.entries(depositTypes)) {
    if (grid.some(row => row.some(cell => info.terrain.includes(cell)))) {
      viableTypes.push({ name, ...info });
    }
  }

  if (viableTypes.length === 0) return [];

  const numDeposits = 1 + Math.floor(rng() * 3);
  const deposits = [];
  const used = new Set();
  ...
  for (let d = 0; d < numDeposits && d < viableTypes.length; d++) {
    ...
    deposits.push({ grid_x: center.x, grid_y: center.y, resource_type: depositType.name, bonus: depositType.bonus });
    ...
    for (let i = 1; i < clusterSize; i++) {
      ...
      if (nx >= 1 && nx < width - 1 && ny >= 1 && ny < height - 1 && !used.has(`${nx},${ny}`)) {
        deposits.push({ grid_x: nx, grid_y: ny, resource_type: depositType.name, bonus: depositType.bonus });
        used.add(`${nx},${ny}`);
      }
    }
  }
...
export { PLANET_TYPE_TO_PROFILE, TERRAIN_PROFILES, BUILDABLE_TERRAIN };
